...
void appAccel() {
  ...
  while (!ttgo->getTouch(x, y)) { // Wait for touch to exit
    ...
    xpos = acc.x;
    ypos = acc.y;
    ttgo->tft->fillCircle(xpos / 10 + 119, ypos / 10 + 119, 10,
                          TFT_RED); // draw dot
    delay(100);
    ttgo->tft->fillCircle(xpos / 10 + 119, ypos / 10 + 119, 10,
                          TFT_BLACK); // erase previous dot
  }
...
// 3) in the main routine in TWatch_framework, add a case to the switch
// statement to call your app routine.

const int maxApp = 6; // number of apps
String appName[maxApp] = {"Clock",   "Jupiter", "Accel",
                          "Battery", "Touch",   "Set Time"}; // app names
...
void appBattery() {
  ...
  ttgo->power->adc1Enable(AXP202_VBUS_VOL_ADC1 | AXP202_VBUS_CUR_ADC1 |
...
void setup() {
  ...
  ttgo->tft->setTextColor(
      TFT_YELLOW,
      TFT_BLACK); // Note: the new fonts do not draw the background colour
